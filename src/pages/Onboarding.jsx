...
import { signUpWithEmail, signInWithEmail, signInWithGoogle } from '../supabase';
...
export default function Onboarding() {
  ...
  const Input = ({ label, placeholder, type = 'text', onChange }) => (
    <div style={{ marginBottom: 13 }}>
      ...
      <input type={type} placeholder={placeholder} onChange={onChange}
        style={{ width: '100%', padding: '11px 14px', background: 'var(--inp)', border: '1.5px solid var(--border)', borderRadius: 10, color: 'var(--text)', fontSize: 14, fontFamily: 'Inter,sans-serif', outline: 'none' }}
  ...
  const BtnP = ({ children, onClick, disabled }) => (
    <button onClick={onClick} disabled={disabled || loading} style={{
      width: '100%', padding: 13, background: 'var(--pg)', border: 'none', borderRadius: 11,
      color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter,sans-serif',
      marginBottom: 10, opacity: (disabled || loading) ? .6 : 1, transition: 'all .2s'
    }}>{loading ? 'Please wait...' : children}</button>
  );

  const BtnS = ({ children, onClick }) => (
    <button onClick={onClick} style={{
      width: '100%', padding: 12, background: 'var(--inp)', border: '1.5px solid var(--border)',
      borderRadius: 11, color: 'var(--text)', fontSize: 14, cursor: 'pointer',
      fontFamily: 'Inter,sans-serif', marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10
    }}>{children}</button>
  );
  ...
  const Back = ({ onClick }) => (
    <button onClick={onClick} style={{ background: 'none', border: 'none', color: 'var(--text3)', fontSize: 12, cursor: 'pointer', padding: 0, fontFamily: 'inherit', marginBottom: 18 }}>← Back</button>
  );

  const Err = () => error ? <div style={{ background: 'var(--reds)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: 9, padding: '10px 13px', fontSize: 12, color: 'var(--red)', marginBottom: 12 }}>{error}</div> : null;
  ...
  const steps = [
    ...
    <div key={0} style={{ animation: 'fadeUp .28s ease' }}>
      ...
      <BtnS onClick={handleGoogleLogin}><GoogleIcon /> Continue with Google</BtnS>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '14px 0', color: 'var(--text3)', fontSize: 11 }}>
        <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />or<div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
      </div>
      <BtnP onClick={() => { setError(''); setStep(1); }}>Create an account</BtnP>
      <BtnS onClick={() => { setError(''); setStep(3); }}>Sign in to existing account</BtnS>
    ...
    <div key={1} style={{ animation: 'fadeUp .28s ease' }}>
      ...
      <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 20, lineHeight: 1.5 }}>Pick your role. This shapes your workspace permissions.</p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 20 }}>
        {ROLES.map(r => (
          <div key={r.id} onClick={() => setRole(r.id)} style={{
            border: `1.5px solid ${role === r.id ? 'var(--p)' : 'var(--border)'}`, borderRadius: 13,
            padding: '15px 12px', cursor: 'pointer', background: role === r.id ? 'var(--ps)' : 'var(--inp)', textAlign: 'center'
          }}>
    ...
    <div key={2} style={{ animation: 'fadeUp .28s ease' }}>
      ...
      <h2 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-1px', marginBottom: 6 }}>Create account</h2>
      <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 16, lineHeight: 1.5 }}>Your phone number stays private — never shown to anyone.</p>
      <Err />
      ...
      <Input label="Institution / Organisation" placeholder="Delhi University" onChange={set('institution')} />
      <Input label="Password" placeholder="Min. 6 characters" type="password" onChange={set('password')} />
      <BtnP onClick={handleSignUp}>Create account →</BtnP>
      <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--text2)' }}>
        Already have an account? <button onClick={() => setStep(3)} style={{ background: 'none', border: 'none', color: 'var(--p)', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Sign in</button>
      </div>
    </div>,
    ...
    <div key={3} style={{ animation: 'fadeUp .28s ease' }}>
      ...
      <Err />
      <BtnS onClick={handleGoogleLogin}><GoogleIcon /> Continue with Google</BtnS>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '12px 0', color: 'var(--text3)', fontSize: 11 }}>
        <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />or<div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
      </div>
      <Input label="Email address" placeholder="rahul@college.edu" type="email" onChange={set('email')} />
      ...
      <div style={{ textAlign: 'right', marginBottom: 14, marginTop: -4 }}>
        <button style={{ background: 'none', border: 'none', color: 'var(--p)', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Forgot password?</button>
      </div>
      <BtnP onClick={handleSignIn}>Sign in →</BtnP>
      <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--text2)' }}>
    ...
    // 4: Email verify
    <div key={4} style={{ animation: 'fadeUp .28s ease', textAlign: 'center', padding: '20px 0' }}>
      <div style={{ fontSize: 50, marginBottom: 16 }}>📧</div>
      <h2 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-1px', marginBottom: 8 }}>Check your email</h2>
      <p style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6, marginBottom: 20 }}>
        We sent a verification link to <strong style={{ color: 'var(--p)' }}>{form.email}</strong>. Click the link to activate your account then come back and sign in.
      </p>
      <BtnP onClick={() => setStep(3)}>Go to sign in →</BtnP>
    </div>
  ];
    ...
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 460px', overflow: 'auto' }}>
      ...
      <div style={{ background: 'var(--bg)', padding: '44px 52px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <h1 style={{ fontSize: 48, fontWeight: 900, lineHeight: 1.05, letterSpacing: '-2.5px', marginBottom: 16 }}>
            Every class<br />needs a<br />
            <span style={{ background: 'var(--pg)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Room.</span>
          </h1>
          <p style={{ fontSize: 15, color: 'var(--text2)', lineHeight: 1.7, marginBottom: 38, maxWidth: 420 }}>
            Replaces WhatsApp groups, missed emails, and paper attendance — for classrooms and teams. Your phone number stays private. Always.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 13, background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 13, padding: '13px 16px' }}>
